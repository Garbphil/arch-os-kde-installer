...
properties_generate() {
    { # Write properties to installer.conf
        ...
        echo "ARCH_OS_DESKTOP_EXTRAS_ENABLED='${ARCH_OS_DESKTOP_EXTRAS_ENABLED}' # Enable desktop extra packages (caution: if disabled, only core + plasma + git packages will be installed) | Disable: false"
        echo "ARCH_OS_DESKTOP_SLIM_ENABLED='${ARCH_OS_DESKTOP_SLIM_ENABLED}' # Enable Slim Desktop (only KDE Core Apps) | Default: false"
...
properties_preset_source() {
    ...
    if [ -f "$SCRIPT_CONFIG" ]; then
        ...
        options=("desktop - KDE Plasma Desktop Environment (default)" "core    - Minimal Arch Linux TTY Environment" "none    - No pre-selection")
...
select_enable_desktop_environment() {
    if [ -z "$ARCH_OS_DESKTOP_ENABLED" ]; then
        local user_input
        gum_confirm "Enable KDE Plasma Desktop Environment?"
        local user_confirm=$?
        [ $user_confirm = 130 ] && {
            trap_gum_exit_confirm
            return 1
...
select_enable_desktop_slim() {
    if [ "$ARCH_OS_DESKTOP_ENABLED" = "true" ]; then
        if [ -z "$ARCH_OS_DESKTOP_SLIM_ENABLED" ]; then
            local user_input
            gum_confirm "Enable Desktop Slim Mode? (KDE Core Apps only)" --affirmative="No (default)" --negative="Yes"
            local user_confirm=$?
            [ $user_confirm = 130 ] && {
                trap_gum_exit_confirm
                return 1
...
exec_install_desktop() {
    local process_name="KDE Plasma Desktop"
    if [ "$ARCH_OS_DESKTOP_ENABLED" = "true" ]; then
        process_init "$process_name"
        (
            [ "$DEBUG" = "true" ] && sleep 1 && process_return 0 # If debug mode then return
            ...
            # Force remove KDE apps
            if [ "$ARCH_OS_DESKTOP_SLIM_ENABLED" = "true" ]; then
                chroot_pacman_remove angelfish || true
                chroot_pacman_remove tokodon || true
                chroot_pacman_remove neochat || true
                chroot_pacman_remove kasts || true
                chroot_pacman_remove klettres || true
                chroot_pacman_remove kwordquiz || true
                chroot_pacman_remove kalm || true
                chroot_pacman_remove artikulate || true
            ...
            mkdir -p /mnt/etc/sddm.conf.d/
            {
                ...
                echo ""
                echo "[General]"
                echo "HaltCommand=/usr/bin/systemctl poweroff"
                echo "RebootCommand=/usr/bin/systemctl reboot"
            ...
            {
                echo '# SSH AGENT'
                echo 'SSH_AUTH_SOCK=$XDG_RUNTIME_DIR/ssh-agent.socket' # KDE Wallet SSH agent
                echo ''
                echo '# PATH'
            ...
            arch-chroot /mnt systemctl enable sddm.service      # SDDM
            ...
            # arch-chroot /mnt /usr/bin/runuser -u "$ARCH_OS_USERNAME" -- systemctl enable --user ssh-agent.service      # KDE Wallet SSH
            ...
            if [ "$ARCH_OS_DESKTOP_EXTRAS_ENABLED" = "true" ]; then
                {
                    echo "# exec_install_desktop | Favorite apps (KDE Task Manager)"
                    echo "kwriteconfig5 --file ~/.config/plasma-org.kde.plasma.desktop-appletsrc --group Containments --group 1 --group Applets --group 2 --group Configuration --group General --key favorites \"['org.kde.dolphin.desktop','org.kde.konsole.desktop','org.kde.kate.desktop','org.kde.systemsettings.desktop']\""
                    echo "# exec_install_desktop | Theming settings"
                    echo "kwriteconfig5 --file ~/.config/kdeglobals --group General --key ColorScheme BreezeLight"
                    echo "plasma-apply-colorscheme BreezeLight"
                    echo "plasma-apply-icontheme Papirus"
                    echo "# exec_install_desktop | Font settings"
                    echo "kwriteconfig5 --file ~/.config/kdeglobals --group General --key XftHinting slight"
                    echo "kwriteconfig5 --file ~/.config/kdeglobals --group General --key XftAntialias true"
                    echo "kwriteconfig5 --file ~/.config/kdeglobals --group General --key font 'Monospace,FiraCode Nerd Font,11,-1,5,50,0,0,0,0,0'"
                    echo "# exec_install_desktop | KWin settings"
                    echo "kwriteconfig5 --file ~/.config/kwinrc --group Windows --key Placement Centered"
                    ...
                    echo "kwriteconfig5 --file ~/.config/kglobalshortcutsrc --group kwin --key 'Window Close' 'Meta+Q,Meta+F4,Close Window'"
                    echo "kwriteconfig5 --file ~/.config/kglobalshortcutsrc --group kwin --key 'Window Minimize' 'Meta+H,none,Minimize'"
                    echo "kwriteconfig5 --file ~/.config/kglobalshortcutsrc --group kwin --key 'Show Desktop' 'Meta+D,none,Show Desktop'"
                    echo "kwriteconfig5 --file ~/.config/kglobalshortcutsrc --group kwin --key 'Window Fullscreen' 'Meta+F11,none,Fullscreen'"
    ...
    if [ -n "$ARCH_OS_DESKTOP_GRAPHICS_DRIVER" ] && [ "$ARCH_OS_DESKTOP_GRAPHICS_DRIVER" != "none" ]; then
        ...
        (
            ...
            "nvidia") # https://wiki.archlinux.org/title/NVIDIA#Installation
                ...
                # Enable Wayland Support for KDE with NVIDIA
                echo 'export KWIN_DRM_NO_AMS=1' >> /mnt/etc/environment
